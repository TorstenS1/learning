...
db_service = get_db_service(use_simulator=True)
...
def run_workflow_step(state: ALISState, start_node: str) -> Dict[str, Any]:
    ...
    for step_output in workflow.stream(state, {"recursion_limit": 10}):
...
def get_material():
    ...
    try:
        ...
        # For material generation, we need to call the node directly
